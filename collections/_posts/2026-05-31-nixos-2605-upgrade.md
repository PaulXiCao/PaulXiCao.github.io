...
1. Rename `programs.vscode` to `programs.vscodium` (and drop the redundant `package = pkgs.vscodium` — it's the default for `programs.vscodium`).
2. Remove `anthropic.claude-code` from the extensions list. The extension build was broken, and I already had the Claude Code CLI installed as a system package via `unstable.claude-code` anyway.
...
| `claude-code` extension build failure (exit 127) | Broken derivation in 26.05 | Remove extension; use CLI package instead |
